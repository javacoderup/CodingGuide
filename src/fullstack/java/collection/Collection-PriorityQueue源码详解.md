...
-   [PriorityQueue源码解析](#collection---priorityqueue%E6%BA%90%E7%A0%81%E8%A7%A3%E6%9E%90)
    -   [概述](#%E6%A6%82%E8%BF%B0)
    -   [方法剖析](#%E6%96%B9%E6%B3%95%E5%89%96%E6%9E%90)
        -   [add()和offer()](#add%E5%92%8Coffer)
        -   [element()和peek()](#element%E5%92%8Cpeek)
        -   [remove()和poll()](#remove%E5%92%8Cpoll)
...
Java中PriorityQueue实现了Queue接口，不允许放入`null`元素；其通过堆实现，具体说是通过完全二叉树(complete binary tree)实现的**小顶堆**(任意一个非叶子节点的权值，都不大于其左右子节点的权值)，也就意味着可以通过数组来作为PriorityQueue的底层实现。

![底层实现](/assets/images/java/PriorityQueue_base.png)
...
PriorityQueue的`peek()`和`element`操作是常数时间，`add()`, `offer()`, 无参数的`remove()`以及`poll()`方法的时间复杂度都是log(N)。
...
![添加元素实现.png](/assets/images/java/PriorityQueue_offer.png)
...
![获取元数实现.png](/assets/images/java/PriorityQueue_peek.png)
...
![poll底层实现.png](/assets/images/java/PriorityQueue_poll.png) 
...
`remove(Object o)`方法用于删除队列中跟`o`相等的某一个元素(如果有多个相等，只删除一个)，该方法不是Queue接口内的方法，而是Collection接口的方法。由于删除操作会改变队列结构，所以要进行调整；又由于删除元素的位置可能是任意的，所以调整过程比其它函数稍加繁琐。具体来说，`remove(Object o)`可以分为2种情况: 1. 删除的是最后一个元素。直接删除即可，不需要调整。2. 删除的不是最后一个元素，从删除点开始以最后一个元素为参照调用一次`siftDown()`即可。此处不再赘述。

![队列删除底层实现.png](/assets/images/java/PriorityQueue_remove2.png)
